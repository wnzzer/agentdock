 ...
 */
const modelDefault = computed(() => !currentModelEntry.value || currentModelEntry.value.isDefault === true);
...
const effortName = computed(() => liveEffort.value ? effortLabel(liveEffort.value) : t('Thinking depth'));
/** Inside the panel there is room to say what the chip cannot: that nothing has
 * been chosen and the client is deciding. */
const effortSetting = computed(() => liveEffort.value ? effortLabel(liveEffort.value) : t('Client default'));
