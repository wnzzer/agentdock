...
const effortIndex = computed(() => Math.max(0, effortStops.value.indexOf(selectedEffort.value)));
const effortName = computed(() => selectedEffort.value ? effortLabel(selectedEffort.value) : t('Endpoint default'));
...
function dragEffort(event: Event) {
  ...
  selectedEffort.value = stop; chooseEffort();
}
...
const effortVivid = computed(() => !!selectedEffort.value && effortStops.value.length > 2 && effortIndex.value >= effortStops.value.length - 2);
...
const MarkdownContent = defineComponent({
  ...
  setup(markdownProps) {
    const inline = (text: string) => markdownInline(text).map((part: MarkdownInline) => part.type === 'link' ? h('a', { href: part.href, target: '_blank', rel: 'noopener noreferrer' }, part.text) : part.type === 'text' ? part.text : h(part.type === 'strong' ? 'strong' : part.type === 'em' ? 'em' : 'code', part.text));
    return () => h('div', { class: 'chat-markdown' }, markdownBlocks(markdownProps.text).map(block => block.type === 'code' ? h('div', { class: 'chat-code' }, [block.language ? h('small', block.language) : null, h('pre', [h('code', block.text)])]) : block.type === 'list' ? h(block.ordered ? 'ol' : 'ul', block.items.map(item => h('li', inline(item)))) : h(block.type === 'heading' ? 'h' + Math.min(block.level ?? 3, 6) : block.type === 'quote' ? 'blockquote' : 'p', inline(block.text))));
  },
});
...
function chooseEffort() { endpointConfirm.value = !!selectedEffort.value || selectedProfile.value !== (props.session.endpoint_profile_id ?? ''); }
...
<template>
  <section :class="['chat-pane', { 'has-tab-menu': !!paneId }]" :aria-label="t('Conversation with {provider}', {provider:providerLabel(session.provider)})" @keydown="paneKeydown">
    ...
    <div ref="viewport" class="chat-timeline" role="log" :aria-label="t('Conversation messages')" aria-live="polite" :aria-busy="view.turn==='running'" @scroll="atBottom">
      ...
      <div v-if="view.turn==='running'" class="chat-working"><span /><span /><span />{{ t('Agent is working') }}</div>
    ...
    <form class="chat-composer" @submit.prevent="send">
      ...
      <textarea ref="composerInput" v-model="draft.text" :aria-label="t('Message your agent')" :placeholder="t('Ask your agent to build, explore, or fix something…')" rows="2" :disabled="!supported||mode!=='structured'||!!draft.pending" @compositionstart="composing=true" @compositionend="composing=false" @keydown="keydown" @keyup="syncCaret" @click="syncCaret" @input="syncCaret" />
...
.chat-markdown :deep(h1),.chat-markdown :deep(h2),.chat-markdown :deep(h3){font-size:16px;line-height:1.45;margin:16px 0 8px}
