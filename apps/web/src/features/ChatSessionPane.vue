...
/**
 * The branch this session works on, and a way to give it another one.
 *
 * A branch belongs to a checkout, and every session in a checkout shares it,
 * so switching one session's branch means moving it to a checkout of its own:
 * a worktree. A running client cannot change its directory, so the
 * conversation continues in a new session there -- as with a handoff, what
 * was said is written into its message box -- and this one stays as it was.
 * Switching the whole checkout is the Git pane's job, not a session's.
 */
interface RepoWorktree { path: string; branch?: string | null; main: boolean }
const repoState = ref<{ current?: string | null; branches: string[]; worktrees: RepoWorktree[] }>();
const branchTarget = ref(''), branchMenu = ref<InstanceType<typeof ChipMenu>>();
...
const sessionBranch = computed(() => repoState.value?.current ?? undefined);
const otherBranches = computed(() => (repoState.value?.branches ?? []).filter(name => name !== sessionBranch.value));
async function moveToBranch(branch: string) {
  branch = branch.trim(); branchMenu.value?.close(true);
  if (!branch || actionBusy.value || branch === sessionBranch.value) return;
  actionBusy.value = true; error.value = '';
  try {
    const existing = repoState.value?.worktrees.find(tree => tree.branch === branch);
    const workspace = await request<Workspace>(`${workspacePath(props.session.workspace_id)}/git/worktrees`, json('POST', existing ? { path: existing.path } : { branch, create: !repoState.value?.branches.includes(branch) }));
    branchTarget.value = '';
    handoffTarget.value = { provider: props.session.provider, profileId: props.session.endpoint_profile_id ?? null, name: branch, note: workspace.root_path, workspaceId: workspace.id, branch };
  } catch (cause) { error.value = errorMessage(cause); }
  finally { actionBusy.value = false; }
  if (handoffTarget.value?.branch === branch) await confirmHandoff();
}
...
.chat-branch-body nav button:hover{background:var(--teal-soft)}
...
.chat-branch-body nav small{flex-shrink:0;font-size:10px;color:var(--violet)}
