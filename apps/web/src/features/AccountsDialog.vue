...
import { accountLimitPercent, quotaLevel, accountLoginUrl, accountResetDate, accountSignInActions, accountStatusLabel, accountResetCreditState, canResetAccountQuota, quotaResetAttempts, quotaResetOutcomeNotice, usageRetryHint, QUOTA_RESET_STORAGE_WARNING, type AccountLimit, type AccountView } from './account-ui';
...
.account-mini{display:flex;gap:4px;margin-top:7px}
.account-mini i{--bar:var(--teal);position:relative;flex:1;height:4px;border-radius:3px;background:var(--fill);overflow:hidden}
.account-mini i::after{content:"";position:absolute;inset:0 auto 0 0;width:var(--fill);border-radius:3px;background:var(--bar)}
.account-mini i.warn{--bar:#c88a1c}.account-mini i.danger{--bar:#c2415a}
