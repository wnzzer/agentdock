...
/** The window closest to running out: the one that limits the account now. */
function tightest(account: AccountView) {
  const slots = (['primary', 'secondary'] as const).filter(slot => accountLimitPercent(account.limits?.[slot]) !== undefined);
  if (!slots.length) return undefined;
  const slot = slots.reduce((a, b) => accountLimitPercent(account.limits?.[b])! > accountLimitPercent(account.limits?.[a])! ? b : a);
  const used = accountLimitPercent(account.limits?.[slot])!;
  return { title: shortTitle(slot, account.limits?.[slot]), left: Math.max(0, Math.round(100 - used)), level: quotaLevel(used) };
}
...
<template>
  <ModalDialog :title="t('Official accounts')" wide :closable="!busy" :embedded="embedded" @close="emit('close')">
    ...
    <template v-else>
      ...
      <div v-else class="accounts-layout"><aside class="accounts-list"><div class="accounts-list-heading"><span>{{ t('Accounts') }} <small>{{ accounts.length }}</small></span><button :aria-label="t('Refresh account list')" :disabled="loading||!!busy" @click="load"><Icon name="refresh" :size="16" /></button></div><button v-for="account in accounts" :key="account.id" :class="['account-choice',{selected:selectedId===account.id&&!adding}]" :disabled="!!busy" @click="choose(account.id)"><span :class="['account-provider',account.provider]"><ProviderIcon :provider="account.provider" :size="19" /></span><span class="account-choice-text"><strong>{{ account.name }}</strong><small>{{ account.email??providerLabel(account.provider) }}<template v-if="account.native_source_id"> · {{ t('Imported native config') }}</template></small><span v-if="tightest(account)" class="account-mini" :class="tightest(account)!.level">{{ tightest(account)!.title }} <b>{{ t('{count}% left', { count: tightest(account)!.left }) }}</b></span></span><span :class="['account-dot',account.status]" /></button><p v-if="loading" class="accounts-empty">{{ t('Loading accounts…') }}</p><button class="account-add" :disabled="!!busy" @click="adding=true;importing=false;confirmAction=undefined;error='';notice=''"><Icon name="plus" :size="16" />{{ t('Add account') }}</button><button class="account-add account-import" :disabled="!!busy||!backendCapabilities.nativeConfig" @click="openImport"><Icon name="folder" :size="16" />{{ t('Import existing configuration') }}</button></aside>
